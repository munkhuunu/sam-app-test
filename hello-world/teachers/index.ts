...
const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  try {
    ...
    if (method === 'POST' && path === '/teachers') {
      ...
      const item = {
        PK: `SCHOOL#${body.schoolId}`, SK: `TEACHER#${teacherId}`,
        GSI1PK: `SCHOOL#${body.schoolId}#TEACHERS`, GSI1SK: `TEACHER#${teacherId}`,
        GSI2PK: `TEACHER#${teacherId}`, GSI2SK: `TEACHER#${teacherId}`,
        entityType: 'TEACHER', teacherId, schoolId: body.schoolId,
        firstName: body.firstName, lastName: body.lastName,
        email: body.email ?? null, phone: body.phone ?? null,
        classIds: [], subjectIds: [], createdAt: new Date().toISOString(),
      };
    ...
    if (method === 'POST' && path.endsWith('/assign')) {
      ...
      await docClient.send(new UpdateCommand({
        ...
        ExpressionAttributeValues: { ':cids': Array.from(classIds), ':now': new Date().toISOString() },
      }));
